...
function ResizePicture(){
	...
	bg0Img.resize(1036,560)
	easyImg.resize(1036,560)
	normalImg.resize(1036,560)
	hardImg.resize(1036,560)
	crazyImg.resize(1036,560)
	hellImg.resize(1036,560)
	cloudImg.resize(1200,250)
}
function setup(){
	createCanvas(1036, 560);
	noStroke();
	mySystem = new System();
	mic = new p5.AudioIn();
	mic.start();
...
class Player extends Entity{
	...
	isAlive(){
		return (this.y > -160) && (this.y < height + 160);
	}
	draw(){
...
class Block extends Entity{
	...
	reverse(){
		switch(this.type){
			case "upper":
				if(this.kind !== 2){
					if(this.y < -220 || this.y > 20){ this.vy *= -1; }
				}else{
					if(this.y < -220 || this.y > -80){ this.vy *= -1; }
				}
				break;
			case "lower":
				if(this.kind !== 2){
					if(this.y < 540 || this.y > 780){ this.vy *= -1; }
				}else{
					if(this.y < 640 || this.y > 780){ this.vy *= -1; }
				}
				break;
		}
	}
	...
	draw(){
		if(this.type === "upper"){
			image(this.image, this.x - this.w - 40, this.y - this.h);
		}else{
			image(this.image, this.x - this.w - 40, this.y - this.h - 40);
		}
	}
}
