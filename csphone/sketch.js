...
const prefix="https://raw.githubusercontent.com/HuilinDai/CLAPPY-SQUARE/master/"

function preload(){
	playerImg = loadImage(prefix+'images/square.png');
	bg0Img = loadImage(prefix+'images/background/bg2.jpg')
	easyImg = loadImage(prefix+'images/difficulty/1easy.png')
	normalImg = loadImage(prefix+'images/difficulty/2normal.png')
	hardImg = loadImage(prefix+'images/difficulty/3hard.png')
	crazyImg = loadImage(prefix+'images/difficulty/4crazy.png')
	hellImg = loadImage(prefix+'images/difficulty/5hell.png')
	cloudImg = loadImage(prefix+'images/cloud1.png')
	blockImg = {upper:[], lower:[]};
	for(let i = 0; i < 5; i++){
		const img_upper = loadImage(prefix+"phoneblock/"+"block_upper_" + i + "_phone.png");
		const img_lower = loadImage(prefix+"phoneblock/"+"block_lower_" + i + "_phone.png");
		blockImg.upper.push(img_upper);
		blockImg.lower.push(img_lower);
	}
	soundEffects = {};
	soundEffects.miss = loadSound(prefix+"audios/death.wav");
	soundEffects.decision = loadSound(prefix+"audios/click.wav");
}

//主函数
function ResizePicture(){
	playerImg.resize(35,35)
...
class System{
	...
	createBlock(type){
		switch(type){
			case 0:
				const y0 = random(-220, 20);
				this.blockArray.push(new Block(width + 80, y0, -2, 0, "upper", 0));
				this.blockArray.push(new Block(width + 80, y0 + 760, -2, 0, "lower", 0));
				break;
			case 1:
                this.createSimpleBlock(0.5, 1);
				break;
			case 2:
				const y2 = random(-200, -100);
				this.blockArray.push(new Block(width + 80, y2, -2, 1, "upper", 2));
				this.blockArray.push(new Block(width + 80, 560 - y2, -2, -1, "lower", 2));
				break;
			case 3:
	...
	createSimpleBlock(v, type){
		...
		this.blockArray.push(new Block(width + 80, y, -2, vy, "upper", type));
		this.blockArray.push(new Block(width + 80, y + 760, -2, vy, "lower", type));
	}
	createCloud(){
	...
	playUpdate(){
		...
		for(let cloud of this.cloudArray){ cloud.update(); }
		if(this.properFrameCount % 160 === 0){ this.createBlock(this.getType()); }
		if(this.properFrameCount % 120 === 0){ this.createCloud(); }
		this.player.update();
		if(this.properFrameCount %120 === 0){
	...
	drawChoiceCircle(level, currentSelectLevel, x){
		...
		ellipse(x, height * 3 / 4, width / 7, width / 7);
	}
	playDraw(){	
